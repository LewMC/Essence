...
public class UpdateUtil {
    ...
    public void VersionCheck() {
        if (this.plugin.getConfig().getBoolean("update-check")) {
            try {
                URL url = new URL("https://service.lewmc.net/latest-version/?resource=essence&format=simpleversion");
                Scanner s = new Scanner(url.openStream());
                if (s.hasNextLine()) {
                    String response = s.nextLine();
                    if (response.equals("")) {
                        log.warn("Unable to perform update check: There was no response from the server.");
                    } else if (response.equals(this.plugin.getDescription().getVersion())) {
                        log.info("You are running the latest version of Essence.");
                    } else {
                        log.warn("There's a new version of Essence available.");
                        log.warn("Your version: "+this.plugin.getDescription().getVersion()+" - latest version: "+response);
                    }
                }
            } catch (MalformedURLException e) {
                log.warn("Unable to perform update check: MalformedURLException - "+e);
            } catch (IOException e) {
